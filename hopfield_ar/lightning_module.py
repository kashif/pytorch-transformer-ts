...
from module import HopfieldARModel


class HopfieldARLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``HopfieldARModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``HopfieldARModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model_kwargs
        Keyword arguments to construct the ``HopfieldARModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = HopfieldARModel(**model_kwargs)
        self.loss = loss
        self.lr = lr
