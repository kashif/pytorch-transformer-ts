...
estimator = LagGPTEstimator(
    ...
    rope_scaling = (dict(type="linear", factor=2.0) if gpt["gpt"]["rope_scaling"] else None),
...
for i in range(25):
    estimator = LagGPTEstimator(
        ...
        ckpt_path="context-window-scaling-logs/"+ experiment_name + "version_" + str(experiment_version) + "/checkpoints/epoch=49-step=5000.ckpt",
    )
...
pd.DataFrame(crpss, columns=["CRPS"]).to_csv("context-window-scaling-logs/"+experiment_name+"/version_"+str(experiment_version)+"/CRPS_rope.csv")
