...
experiment_name = ("context-window-scaling-rope" if config["gpt"]["rope_scaling"] else "context-window-scaling")
experiment_logger = CSVLogger(save_dir="context-window-scaling-logs", name=experiment_name) if config["metrics"]["logger"]=="csv" else WandbLogger(save_dir="context-window-scaling-logs", name=experiment_name)
experiment_version = experiment_logger.version

print("Running training for "+ experiment_name+ " version "+ str(experiment_version))
...
estimator = LagGPTEstimator(
    ...
    rope_scaling = (dict(type="linear", factor=2.0) if config["gpt"]["rope_scaling"] else None),
    ...
    trainer_kwargs=dict(max_epochs=config["gpt"]["max_epochs"], accelerator="gpu", precision="32", logger=experiment_logger, devices=[config["CUDA"]["device_id"]]),
)
...
print("Running evaluation for "+ experiment_name+ " version "+ str(experiment_version))
...
try:
    for i in range(100):
        ...
        estimator = LagGPTEstimator(
            ...
            batch_size=32,
            num_batches_per_epoch=config["gpt"]["batches_per_epoch"],
            trainer_kwargs=dict(accelerator="gpu", precision="32", max_epochs=0, logger=test_logger, devices=[config["CUDA"]["device_id"]]),
            ckpt_path="context-window-scaling-logs/"+ experiment_name + "version_" + str(experiment_version) + "/checkpoints/*",
        )
        ...
        estimator = LagGPTEstimator(
            ...
            num_batches_per_epoch=config["gpt"]["batches_per_epoch"],
            trainer_kwargs=dict(accelerator="gpu", precision="32", max_epochs=0, logger=test_logger, devices=[config["CUDA"]["device_id"]]),
            ckpt_path="context-window-scaling-logs/"+ experiment_name + "version_" + str(experiment_version) + "/*",
        )
    ...
    ax.set_ylabel("CRPS")
    fig.savefig("context-window-scaling-logs/"+experiment_name+"/version_"+str(experiment_version)+"/CRPS.png")
    plt.close(fig)

    pd.DataFrame(crpss, columns=["CRPS"]).to_csv("context-window-scaling-logs/"+experiment_name+"/version_"+str(experiment_version)+"/CRPS.csv")

    fig, ax = plt.subplots()
    ax.plot([(i+1)*meta.prediction_length for i in range(len(crpss_rope))], crpss_rope)
    ax.set_xlabel("Context Length")
    ax.set_ylabel("CRPS")
    fig.savefig("context-window-scaling-logs/"+experiment_name+"/version_"+str(experiment_version)+"/CRPS_rope.png")
    plt.close(fig)

    pd.DataFrame(crpss_rope, columns=["CRPS"]).to_csv("context-window-scaling-logs/"+experiment_name+"/version_"+str(experiment_version)+"/CRPS_rope.csv")
