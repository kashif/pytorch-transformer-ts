...
for seed in seed_list:
    ...
    all_value = []
    value = []
    for layer in [4096]:#: [2,4,8 12,16, 20, 24, 28,32, 46, 64, 74, 76]:#, 20, 24, 28, 32, 46, 64, 74, 76]:
        ...
        estimator = TransformerEstimator(
            #freq=dataset.metadata.freq,
                ...
                trainer_kwargs=dict(max_epochs=5, accelerator='gpu', gpus=6, strategy='ddp',num_nodes = 50, logger=logger, gradient_clip_val=5, callbacks=[checkpoint_callback]))##
        ...
        predictor.serialize(Path("logs_aug/4096/"))
        ...
        df.to_csv('AugSeed_50_'+str(seed)+'_layer'+str(layer)+'.csv', index=False)
