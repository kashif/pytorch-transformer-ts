...
experiment_name = (
    "data-scaling-weighted-" + str(config["gpt"]["aug_prob"])
    if config["dataset"]["weighted"]
    else "data-scaling-uniform-" + str(config["gpt"]["aug_prob"])
)
experiment_logger = (
    CSVLogger(
        save_dir="data-scaling-logs",
        name=experiment_name,
        flush_logs_every_n_steps=config["metrics"]["num_steps"],
    )
    if config["metrics"]["logger"] == "csv"
    else WandbLogger(save_dir="data-scaling-logs", name=experiment_name)
)
...
estimator = LagGPTAlibiEstimator(
    ...
    trainer_kwargs=dict(
        ...
        log_every_n_steps=config["metrics"]["num_steps"],
        val_check_interval=config["metrics"]["num_steps"],
        accelerator="gpu",
        precision="32",
...
predictor = estimator.train(
    training_data=dataset, validation_data=val_dataset, shuffle_buffer_length=1000
)
